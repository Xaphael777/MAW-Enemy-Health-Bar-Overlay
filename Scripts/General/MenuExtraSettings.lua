...
function events.GameInitialized2()
	...
	---------------------------------------------------------
	-- MAW HP OVERLAY CONTROLS
	---------------------------------------------------------
	local overlayPath = AppPath .. "_Python Overlay\\MAW_Overlay.exe"
	local settingsPath = AppPath .. "_Python Overlay\\Overlay_Settings.exe"
	...
	CustomUI.CreateButton{
		...
		Action        = function(t)
			Game.PlaySound(23)
			if fileExists(overlayPath) then
				os.execute('start "" "' .. overlayPath .. '"')
			else
				Game.EscMessage("Overlay.exe not found in _Python Overlay!")
			end
		end
	}
	...
	CustomUI.CreateButton{
		...
		Action        = function(t)
			Game.PlaySound(24)
			os.execute('taskkill /IM MAW_Overlay.exe /F >nul 2>&1')
		end
	}

	-- 3. HP Bar Settings
	CustomUI.CreateText{
		Text   = "HP Bar Settings",
		Layer  = 0,
		Screen = ExSetScr,
		X      = 380, 
		Y      = 255,
	...
	CustomUI.CreateButton{
		...
		Action        = function(t)
			...
			if fileExists(settingsPath) then
				os.execute('start "" "' .. settingsPath .. '"')
			else
				Game.EscMessage("Overlay_Settings.exe not found in _Python Overlay!")
			end
		end
	}
